...
diana::FootStepGenerator g_foot_stp_generator;

diana_online_walking_module_msgs::AddStepDataArray     add_step_data_array_srv;

diana_foot_step_generator::FootStepCommand last_command;
...
void initialize(void)
{
  ...
  g_walking_command_sub           = nh.subscribe("/heroehs/diana_foot_step_generator/walking_command", 0, walkingCommandCallback);
  //g_footsteps_2d_sub              = nh.subscribe("/heroehs/diana_foot_step_generator/footsteps_2d",    0, step2DArrayCallback);
...
void walkingCommandCallback(const diana_foot_step_generator::FootStepCommand::ConstPtr &msg)
{
  ...
  diana_online_walking_module_msgs::GetReferenceStepData    get_ref_stp_data_srv;
  diana_online_walking_module_msgs::StepData                ref_step_data;
  diana_online_walking_module_msgs::AddStepDataArray        add_stp_data_srv;
  ...
  if(g_add_step_data_array_client.call(add_stp_data_srv) == true)
  {
    ...
    if(add_stp_data_srv_result== diana_online_walking_module_msgs::AddStepDataArray::Response::NO_ERROR)
    {
    ...
    else
    {
      ...
      if(add_stp_data_srv_result & diana_online_walking_module_msgs::AddStepDataArray::Response::NOT_ENABLED_WALKING_MODULE)
        ROS_ERROR("[Demo]  : STEP_DATA_ERR::NOT_ENABLED_WALKING_MODULE");
      if(add_stp_data_srv_result & diana_online_walking_module_msgs::AddStepDataArray::Response::PROBLEM_IN_POSITION_DATA)
        ROS_ERROR("[Demo]  : STEP_DATA_ERR::PROBLEM_IN_POSITION_DATA");
      if(add_stp_data_srv_result & diana_online_walking_module_msgs::AddStepDataArray::Response::PROBLEM_IN_TIME_DATA)
        ROS_ERROR("[Demo]  : STEP_DATA_ERR::PROBLEM_IN_TIME_DATA");
      if(add_stp_data_srv_result & diana_online_walking_module_msgs::AddStepDataArray::Response::TOO_MANY_STEP_DATA)
        ROS_ERROR("[Demo]  : STEP_DATA_ERR::TOO_MANY_STEP_DATA");
      if(add_stp_data_srv_result & diana_online_walking_module_msgs::AddStepDataArray::Response::ROBOT_IS_WALKING_NOW)
...
//  diana_online_walking_module_msgs::StepData             ref_step_data;
//  diana_online_walking_module_msgs::AddStepDataArray     add_stp_data_srv;
//  diana_online_walking_module_msgs::IsRunning            is_running_srv;
//
//  if(isRunning() == true)
//    return;
//
